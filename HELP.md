...
2. 分页功能需要查询两次，首先要查询数据总数（即执行select count语句），然后查询当前页数据。

   之后遇到一个问题，使用分页前，如果给example的criteria增加了条件，那么分页之后查询时，会先把条件加到select count语句上，导致无法正常查出数据。
...
1. 删除时将文档下的所有子文档同时删除，并在给予二次提示。目前，删除文档后，子文档也会消失，但并不是数据库中的数据被删除了，而是没有了父文档，在递归遍历时，自然无法根据children找到其子文档。所以，子文档看不到，数据却并没有消失，长此以往，势必基类大量垃圾数据。因此，需要进行优化，做到真正从数据库中把文档及其子文档一并删除。具体做法有两种，要么前端不变，给一个文档id给后端，后端去查询文档表，找到所有parent与文档id一致的条目，逐一删除；要么是前端把所有要删除的id都给后端，后端删除一次即可。前一种要和数据库发生多次交互，效率不高，我们使用后一种，让前端去获取所有要删除的id。
...
1. 在模态对话框里创建了编辑器，却没有显示，原因为何？执行setup时、页面还没有渲染，自然也就没有id为content的标签，无法绑定渲染。那放在onMounted中能行得通吗？还是不行，因为modal标签，一开始并不存在，只有点击了使之弹框，才会出现对应的标签。那就应该把创建编辑器的代码放在弹框的方法中，等modalVisible变为true之后，再去创建。可是，还有一个问题，触发浏览器创建modal时，还需要时间，而且这个时间比我们往下执行代码的时间肯定要短，因此可能还是无法显示，此时就需要为创建编辑器这一步添加一点延迟（如100ms），如此最后才能顺利显示。

   从以上的分析可以看出，前端中涉及的一个重要问题就是加载顺序与时间。只要搞明白组件与变量的创建时机，才能找到问题所在。

文档内容表的设计与代码生成

1. 文档内容不过是文档的一个字段，为什么要另外创建一个专门的表呢？文章内容这个字段比之文档表的其它字段，一般都要长很多，而且不是每次查询都需要使用，可能会数据库造成不必要的压力。将内容单独放一张表，此所谓纵向**分表**，即拿出表中的部分字段放在另外一张表中。而如果按条目将表拆分，如每年的数据单独放一张表，此所谓横向分表。

文档管理页面布局修改，引入栅格

1. 将内容编辑放在模态对话框中，既不方便，也不美观。因此，我们采用左右分栏的形式，将内容编辑器放在页面右侧。
2. 表格的默认展开属性defaultExpandAllRows并不是动态变化的，只会在初始化时执行一次，之后不再执行，因此需要再额外配上v-if，判断level1有数据时才加载，这样才能真正实现默认自动展开。
